...
def histogram(information, title, bins):
    ...
    if title != 'GDP 2':
        textstr = 'Mean: ' + str(round(statistics.mean(information), 2)) + '%\n SD: ' + str(
            round(statistics.stdev(information), 4))
    else:
    ...
    plt.xlabel('Percent')
    plt.ylabel('Count')
    plt.savefig('data/hist' + title, bbox_inches='tight')
    plt.show()
...
def cdf(information, title, marks, vals):
    ...
    plt.xlabel("Spending Percent$(x)$");
    plt.ylabel("$P(X>x)$");
    plt.legend();
    plt.savefig('data/cdf' + title, bbox_inches='tight')
    plt.show()
...
if __name__ == "__main__":
    ...
    vals = [info_dict[item]['Percent Military(of Gov)'] for item in marks if
            info_dict[item]['Percent Military(of Gov)'] != 0]
    vals2 = [info_dict[item]['Percent Military(of GDP)'] for item in marks if
             info_dict[item]['Percent Military(of GDP)'] != 0]
