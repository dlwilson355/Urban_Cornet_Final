...
#        print(mark_x, mark_y)
    ...
    plt.plot(x, line, 'r')
    plt.show()
#    plt.savefig('data/EDA' + x_key + y_key)
    return m
...
def predict_happiness(happiness_level, keys, trends):
    ...
    for item in keys:
        predict[item] = {'no data': 0, 'extremely high': 0, 'high': 0, 'above average': 0, 'below average': 0, 'low': 0,
                         'extremely low': 0}
...
def happy_probabilities(key, predict, title):
    if 'Military' not in key:
        ...
        plt.bar(list(range(len(list(predict[key].values())))[1:]), (list(predict[key].values())[1:]))
    plt.xticks(list(range(len(list(predict[key].values())))[1:]),
...
if __name__ == "__main__":
    ...
    info_dict = read_military(r'data/milt_per_gov.csv', info_dict, TRANSLATE_DICT, '(of Gov)')
    #print(info_dict['Colombia'])
    info_dict = reconcile(info_dict)
    #print(info_dict)
    info_dict = read_military(r'data/milt_GDP_per.csv', info_dict, TRANSLATE_DICT, '(of GDP)')
    #print(info_dict['Colombia'])
    info_dict = reconcile(info_dict)
    ...
    new_gdp_data = load_wealth_data(r'data/gdp_data.csv')
    ...
    poverty_data = load_wealth_data(r'data/poverty_data.csv')
